            ...
            <div class="panel border bg-white">
                ...
                <div class="panel-body p-3">
                    <form @submit.prevent="login()">
                        ...
                        <div class="text-center" v-for="(v, k) in errors" :key="k">
                            <label v-for="error in v" :key="error" class="text-danger" style="font-size: 13px"> {{ error }} </label>
                        </div>

                        <div class="text-center">
                            <label v-if="loginError.error !== ''" class="text-danger" style="font-size: 13px"> {{ loginError.error }} </label>
                        </div>

                        <div class="form-group py-2">
                            <div class="input-field"><input v-model="form.email" type="text" placeholder="Email" required> </div>
                        </div>
                        <div class="form-group py-1 pb-2">
                            <div class="input-field"><input v-model="form.password" type="password" placeholder="Enter your Password" required> <button class="btn bg-white text-muted"> <span class="far fa-eye-slash"></span> </button> </div>
                        </div>
                        <button class="btn btn-primary btn-block mt-3"> Login </button>
                        <div class="text-center pt-4 text-muted">Don't have an account? <router-link :to="{ name: 'UserRegister' }"> Sign up </router-link> </div>
                    </form>
                </div>
...
export default {
    setup() {
        ...
        const { loginUser, errors, loginError } = useUser()
